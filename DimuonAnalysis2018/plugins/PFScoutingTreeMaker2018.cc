...
class PFScoutingTreeMaker2018 : public edm::one::EDAnalyzer<edm::one::SharedResources, edm::one::WatchRuns, edm::one::WatchLuminosityBlocks> {
	public:
		explicit PFScoutingTreeMaker2018(const edm::ParameterSet&);
		~PFScoutingTreeMaker2018();
  ...
  //const edm::EDGetTokenT<std::vector<ScoutingVertex> >    verticesToken;
	...
	std::vector<float>           iso2;
	std::vector<float>           iso3;
...
PFScoutingTreeMaker2018::PFScoutingTreeMaker2018(const edm::ParameterSet& iConfig): 
...
PFScoutingTreeMaker2018::~PFScoutingTreeMaker2018() {
}

void PFScoutingTreeMaker2018::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    using namespace edm;
    using namespace std;
	...
	for (auto pfcands_iter = pfcandsH->begin(); pfcands_iter != pfcandsH->end(); ++pfcands_iter) {
            double dR = deltaR(muons_iter->eta(), muons_iter->phi(), pfcands_iter->eta(), pfcands_iter->phi());
            if (abs(pfcands_iter->pdgId()) != 13) continue;
            muonpt.push_back(pfcands_iter->pt());
	    muoneta.push_back(pfcands_iter->eta());
        }
    ...
    vector<TLorentzVector> pfmuons;
    for (auto pfcands_iter = pfcandsH->begin(); pfcands_iter != pfcandsH->end(); ++pfcands_iter) {
      // double dR = deltaR(muons_iter->eta(), muons_iter->phi(), pfcands_iter->eta(), pfcands_iter->phi());
            if (abs(pfcands_iter->pdgId()) != 13 || pfcands_iter->vertex() != 0) continue;
            muonpt.push_back(pfcands_iter->pt());
	    muoneta.push_back(pfcands_iter->eta());
	    muonphi.push_back(pfcands_iter->phi());
	    TLorentzVector temp;
	    temp.SetPtEtaPhiM(pfcands_iter->pt(),pfcands_iter->eta(),pfcands_iter->phi(),0.106) ;
	    pfmuons.push_back(temp);
	  ...
	  for (auto pfcands_iter2 = pfcandsH->begin(); pfcands_iter2 != pfcandsH->end(); ++pfcands_iter2) {
	  double dR = deltaR(temp.Eta(), temp.Phi(), pfcands_iter2->eta(), pfcands_iter2->phi());
            if (abs(pfcands_iter2->pdgId()) == 11) continue;
            if (abs(pfcands_iter2->pdgId()) == 13) continue;
            if (dR > 0.4) continue;
            if (dR < 0.01) continue;

            if (abs(pfcands_iter2->pdgId()) == 211) cpisoval += pfcands_iter2->pt();
            if (abs(pfcands_iter2->pdgId()) == 211 && pfcands_iter2->vertex() == 0) chisoval += pfcands_iter2->pt();
            if (abs(pfcands_iter2->pdgId()) == 211 && pfcands_iter2->vertex()  > 0) puisoval += pfcands_iter2->pt();
            if (pfcands_iter2->pdgId() == 130) nhisoval += pfcands_iter2->pt();
            if (pfcands_iter2->pdgId() ==  22) phisoval += pfcands_iter2->pt();
        }

	double isoval2 = chisoval + max(0., nhisoval + phisoval - 0.5*puisoval);
	
	double EffectiveArea = 0.0;
	double SCEta = temp.Eta();
	//taken from https://github.com/cms-sw/cmssw/blob/480353f051b2d40a781a9c11512a6de060fd2d35/EgammaAnalysis/ElectronTools/interface/ElectronEffectiveArea.h
	...
	double isoval3 = chisoval + max(0.,nhisoval + phisoval - rho*EffectiveArea);
	iso2  .push_back(  isoval2);
	iso3  .push_back(  isoval3);
...
void PFScoutingTreeMaker2018::beginJob() {
    ...
    tree->Branch("iso2"                  , "std::vector<float>"           , &iso2       , 32000, 0);
    tree->Branch("iso3"                  , "std::vector<float>"           , &iso3       , 32000, 0);
    tree->Branch("muonid"               , "std::vector<char>"            , &muonid    , 32000, 0);
    tree->Branch("muonLooseid"          , "std::vector<char>"            , &muonLooseid    , 32000, 0);
}

void PFScoutingTreeMaker2018::endJob() {
}

void PFScoutingTreeMaker2018::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup) {
    // HLT paths
...
void PFScoutingTreeMaker2018::endRun(edm::Run const&, edm::EventSetup const&) {
}

void PFScoutingTreeMaker2018::beginLuminosityBlock(edm::LuminosityBlock const& iLumi, edm::EventSetup const&) {
}

void PFScoutingTreeMaker2018::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {
}

void PFScoutingTreeMaker2018::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
	edm::ParameterSetDescription desc;
	desc.setUnknown();
	descriptions.addDefault(desc);
}

DEFINE_FWK_MODULE(PFScoutingTreeMaker2018);
